...
class CenterNet:
    ...
    def _build_graph(self):
        ...
        with tf.variable_scope('center_detector'):
            ...
            if self.mode == 'train':
                ...
                num_select = tf.cond(tf.shape(scores)[0] > self.top_k_results_output,
                                     lambda: self.top_k_results_output, lambda: tf.shape(scores)[0])
    ...
    def _keypoints_loss(self, keypoints, gbbox_yx, gbbox_y, gbbox_x, gbbox_h, gbbox_w,
        ...
        keyp_penalty_reduce = tf.exp(
            -((gbbox_y - meshgrid_y) ** 2 + (gbbox_x - meshgrid_x) ** 2) / (2 * sigma ** 2))
